...
class GrantPermissionTest extends AbstractApiFeatureTester
{
    protected Role $role;
    ...
    {
        return route('api.config.role.permissions.grant', [$this->role->id, $this->permission->id]);
    }
    ...
    public function test_role_not_found(): void
    {
        $response = $this->getHttpClient($this->getOperationUser())
            ->post(route('api.config.role.permissions.grant', [$this->getFakeRoleId(), $this->permission->id]));
    ...
    {
        $response = $this->getHttpClient($this->getOperationUser())
            ->post(route('api.config.role.permissions.grant', [$this->role->id, $this->getFakePermissionId()]));

        $response->assertNotFound();
        $this->assertApiMessage(PermissionEnum::NOT_FOUND, $response);
    }

    public function test_grant_permission(): void
    {
        $response = $this->getHttpClient($this->getOperationUser())
            ->post($this->getRoute());

        $response->assertOk();
        $this->assertApiMessage(RoleEnum::PERMISSION_GRANTED, $response);
    ...
    public function test_grant_permission_already_granted(): void
    {
