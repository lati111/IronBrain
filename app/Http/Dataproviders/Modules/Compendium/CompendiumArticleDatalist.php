...
use App\Models\Compendium\Docs\ArticleSegment;
...
class CompendiumArticleDatalist extends AbstractCardlist
{
    ...
    {
        $campaign = Campaign::find(Route::input('campaign_uuid'));
        if ($campaign === null) {
            abort($this->respond(Response::HTTP_BAD_REQUEST, ResponseStrings::CAMPAIGN_NOT_FOUND, Route::input('campaign_uuid')));
        }

        $player = $campaign->findPlayerByUser(Auth::user()->uuid);
        if ($player === null) {
            abort($this->respond(Response::HTTP_FORBIDDEN, 'You are not a player in this campaign.', Auth::user()->uuid));
        }

        $article = Article::find(Route::input('article_uuid'));
        if ($article === null) {
            abort($this->respond(Response::HTTP_BAD_REQUEST, ResponseStrings::ARTICLE_NOT_FOUND, Route::input('article_uuid')));
        }

        if ($article->playerHasAccess($player) === false) {
            abort($this->respond(Response::HTTP_FORBIDDEN, 'You do not have access to this article.', Auth::user()->uuid));
        }

        /** @var Builder $query */
        $query = ArticleSegment::select([sprintf('%s.*', ArticleSegment::TABLE_NAME)])
            ->jointable(Article::getTableName(), ArticleSegment::getTableName(), 'article_uuid', '=', 'uuid')
            ->where('article_uuid', $article->uuid);

        // Limit access to private articles
        $query->where(column: function($q) use ($player) {
            // Allow public segments
            $q->where(sprintf('%s.private', ArticleSegment::TABLE_NAME), false);

            // Allow DM access
            if ($player->is_dm === false) {
                $q->where(sprintf('%s.dm_only', ArticleSegment::TABLE_NAME), false);
            }

            // Limit private articles
            $q->orWhere(function($q) use ($player) {
                $q
                    ->where(sprintf('%s.private', ArticleSegment::TABLE_NAME), true)
